...
import { LatestSessionGuard } from '../latestSession.guard';
import { authHeader, sessionID } from '../mocks';

describe('LatestSessionGuard', () => {
    let guard: LatestSessionGuard;
    let userEntity: UserEntity;

    beforeAll(async () => {
        const module = await Test.createTestingModule({
            providers: [LatestSessionGuard, { provide: UserEntity, useValue: { getUserBySessionID: jest.fn() } }],
        }).compile();

        guard = module.get<LatestSessionGuard>(LatestSessionGuard);
